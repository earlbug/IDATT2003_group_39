...
/**
 * MonopolyController extends GameController and is responsible for controlling monopoly-specific
 * game mechanics. It provides methods for checking if a player has lost, and if the winning
 * condition for monopoly is met.
 */
public class MonopolyController extends GameController {
   ...
   * player to be considered victorious. Specific conditions for monopoly is if there are only one
   * player left, and it is the provided Player.
   *
  ...
  public boolean isWinConditionMet(Player player) {
    List<Player> playersLeft = boardGame.getPlayers().stream().filter(p -> !p.hasLost()).toList();
  ...
  /**
   * An override of the super class method, to check if monopoly specific conditions are met for a
   * player to be considered a looser. Specific conditions for monopoly is if The player is in
   * debt.
   *
